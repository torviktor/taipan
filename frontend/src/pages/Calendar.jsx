...
export default function CalendarPage() {
  ...
  async function loadEvents() {
    try {
      ...
      setEvents(r.data.map(e => ({
        id: e.id, title: e.title, start: e.event_date,
        extendedProps: e, backgroundColor: '#CC0000', borderColor: '#990000',
      })))
    } catch { setEvents([]) }
  }
  ...
  async function deleteEvent() {
    try {
      await axios.delete(`${API}/${deleteConfirm}`, { headers: { Authorization: `Bearer ${token()}` } })
    ...
    <main className="calendar-page">
      <section className="calendar-hero">
        <div className="container">
          ...
          <p className="calendar-sub">Тренировки, соревнования, сборы — все события клуба в одном месте</p>
        </div>
      </section>
      ...
      <div className="container calendar-body">
        {isAdmin() && (
          <div className="calendar-toolbar">
            <button className="btn-primary" onClick={() => openCreate(null)}>+ Создать событие</button>
            <span className="calendar-hint">Кликни на дату в календаре чтобы создать событие</span>
          </div>
        )}
        ...
        <div className="subscribe-hint">
          <div className="subscribe-hint-icon"><TelegramIcon /></div>
          ...
          <div className="modal">
            ...
            <form className="modal-form" onSubmit={saveEvent}>
                ...
                <div className="notify-options">
                  {NOTIFY_OPTIONS.map(opt => (
                    <button key={opt.days} type="button"
                      ...
                      onClick={() => toggleNotifyDay(opt.days)}>{opt.label}</button>
                  ))}
                </div>
                <p className="form-hint">
                  Выбрано: {form.notify_before_days.length === 0 ? 'без напоминаний'
              ...
              <div className="modal-actions">
                {editEvent && (
                  <button type="button" className="btn-delete" onClick={() => setDeleteConfirm(editEvent.id)}>Удалить</button>
                )}
