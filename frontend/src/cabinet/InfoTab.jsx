  ...
  return (
    <div>
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:28 }}>
        ...
        {isManager && !isAdmin && <SectionBtn id="fees_guide" label="Взносы"/>}
        {isAdmin   && <SectionBtn id="fees_admin" label="Взносы"/>}
        ...
        <div>
          ...
          <div style={{ overflowX:'auto', marginBottom:18 }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.92rem' }}>
              ...
              <tbody>
                {[
                  ['Клубный / Местный',        '1.2',  '1.0',  '1.5',  '1.5'],
                  ['Городской / Региональный', '3.0',  '2.5',  '4.0',  '5.0'],
                  ['Окружной',                 '5.0',  '5.0',  '6.0',  '6.0'],
      ...
      {/* ── ВЗНОСЫ (старый раздел — устарел, заменён fees_guide/fees_admin) ── */}
      {section === 'fees' && isManager && false && (
        <div>
      ...
      {/* ── ВЗНОСЫ — МЕНЕДЖЕР ── */}
      {section === 'fees_guide' && isManager && !isAdmin && (
        <div>
          <H2>Управление взносами</H2>

          <div style={{ background:'var(--dark2)', border:'1px solid var(--gray-dim)', borderRadius:10, padding:'18px 22px', marginBottom:24 }}>
            <P>Доступ к панели взносов предоставлен вам тренером клуба. Общую картину по всем группам видит только тренер.</P>
          </div>

          <H3>Как работает система</H3>
          <P>Каждый месяц в день сбора взносов (настраивается тренером) формируется список спортсменов вашей группы. Вы распределяете их на бюджетников и внебюджетников, сохраняете список и при необходимости отправляете уведомления.</P>

          <H3>Бюджетники и внебюджетники</H3>
          <P>Бюджетник — спортсмен освобождённый от оплаты взноса. Внебюджетник — оплачивает взнос в установленном размере. Статус выставляется ежемесячно — динамика переходов сохраняется.</P>

          <H3>Порядок работы каждый месяц</H3>
          <P>1. Дождитесь когда наступит новый месяц — список формируется автоматически.</P>
          <P>2. Распределите спортсменов: поставьте галочку «Бюджетник» тем кто освобождён.</P>
          <P>3. Нажмите «Сохранить список» — изменения сохранятся.</P>
          <P>4. Нажмите «Сохранить и уведомить внебюджетников» — все внебюджетники получат уведомление об оплате в личный кабинет и Telegram.</P>
          <P>5. Когда родитель оплатил — отметьте галочку «Оплачено» напротив спортсмена.</P>

          <H3>Долги</H3>
          <P>Если взнос не оплачен до конца месяца — долг автоматически переносится в следующий месяц и суммируется. При оплате в текущем месяце все долги за прошлые периоды погашаются.</P>

          <H3>Комментарии</H3>
          <P>В таблице есть поле для комментария к каждому спортсмену — используйте его для заметок (например: «оплатит на следующей неделе»).</P>

          <H3>Просмотр истории</H3>
          <P>Прошлые месяцы заморожены и доступны только для просмотра. Используйте стрелки ← → для навигации по месяцам. Переход в будущие месяцы недоступен.</P>
        </div>
      )}

      {/* ── ВЗНОСЫ — АДМИН ── */}
      {section === 'fees_admin' && isAdmin && (
        <div>
          <H2>Взносы — обзор системы</H2>

          <div style={{ background:'var(--dark2)', border:'1px solid var(--gray-dim)', borderRadius:10, padding:'18px 22px', marginBottom:24 }}>
            <P>Учёт взносов ведётся менеджерами в их личных кабинетах. Каждый менеджер работает только со своей группой. Вы видите сводную картину по всем группам — без возможности редактировать данные.</P>
          </div>

          <H3>Что видите вы</H3>
          <P>В вашей вкладке «Взносы» доступен просмотр по всем группам через фильтр: Все группы / Младшая / Старшая / Взрослые. Таблица показывает статус каждого спортсмена: бюджетник или внебюджетник, оплачено или нет, сумму долга и комментарии. Редактирование недоступно — только контроль.</P>

          <H3>Что делают менеджеры</H3>
          <P>Каждый месяц менеджер формирует список своей группы, распределяет спортсменов на бюджетников и внебюджетников, отмечает оплаты и отправляет уведомления должникам. Долги автоматически переносятся на следующий месяц.</P>

          <H3>Настройки</H3>
          <P>День сбора взносов и сумма взноса настраиваются в верхней части вкладки «Взносы». Эти параметры общие для всех групп и менеджеров.</P>

          <H3>Группы менеджеров</H3>
          <P>Группа менеджера определяется автоматически при первом входе во вкладку Взносы — по группе его спортсмена. Младшая группа и Старшая+Взрослые обслуживаются раздельно.</P>
        </div>
      )}
