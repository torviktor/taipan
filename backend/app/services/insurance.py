...
def _text_for(athlete, days_left: int):
    ...
    ПРО ГРАММАТИКУ. Имя стоит ОТДЕЛЬНОЙ СТРОКОЙ в именительном падеже, а не
    внутри фразы. Первая версия писала «У Акулова Ульяна Александровна
    страховка закончилась» — склонять русские ФИО программно нельзя, любая
    попытка спотыкается на фамилиях вроде Кучма, Гнатюк или Белых.

    По той же причине здесь нет ни одного слова, согласуемого с полом.
    Первая версия писала «ребёнок не будет допущена»: род был согласован со
    спортсменкой, а подлежащим в предложении оказался «ребёнок». Безличное
    «к соревнованиям не допускают» снимает вопрос целиком.
    """
    exp = athlete.insurance_expiry
    who = athlete.full_name

    common = ("Без действующей страховки к соревнованиям не допускают.")

    if days_left > 0:
        title = f"Страховка заканчивается через {days_left} дн."
        body = (f"{who}\n"
                f"Страховка действует до {exp:%d.%m.%Y} — осталось {days_left} дн.\n\n"
                f"{common} Если полис уже продлён, покажите его тренеру, "
                "чтобы обновить дату.")
    else:
        title = "Страховка закончилась"
        body = (f"{who}\n"
                f"Страховка закончилась {exp:%d.%m.%Y}.\n\n"
                f"{common} Продлите полис и покажите его тренеру.")
    return title, body
