...
# Что делать дальше — одинаково для всех отказов по ссылке, и порядок здесь
# важен. Раньше на первом месте стояло «попросите тренера прислать новую»:
# это самый долгий из возможных путей, он упирается в другого человека и его
# свободное время. Между тем ссылка вообще не нужна — кнопка «Поделиться
# контактом» решает ту же задачу сразу и без посредников. Поэтому сначала
# она, потом ручной ввод номера, и только последним — тренер.
WHAT_TO_DO = (
    "<b>Ничего страшного, привязаться можно прямо сейчас:</b>\n\n"
    "1. Нажмите /start\n"
    "2. Нажмите кнопку «📱 Поделиться контактом»\n\n"
    "Это займёт пару секунд, ссылка для этого не нужна.\n\n"
    "Если номер в мессенджере отличается от того, которым вы регистрировались "
    "на сайте клуба — отправьте нужный вручную: <code>/link</code> и через "
    "пробел ваш номер с сайта.\n\n"
    "Совсем не выходит — скажите тренеру."
)

REFUSAL_TEXT = {
    "used":    ("🔗 <b>Эта ссылка уже использована.</b>\n\n"
                "Так и задумано: ссылка одноразовая, чтобы пересланная кому-то "
                "ещё она не привязала ваш аккаунт чужому человеку.\n\n"
                + WHAT_TO_DO),
    "expired": ("⌛ <b>Срок действия ссылки истёк.</b>\n\n" + WHAT_TO_DO),
    "unknown": ("🔗 <b>Ссылка не распознана.</b>\n\n"
                "Возможно, она скопирована не целиком.\n\n" + WHAT_TO_DO),
    "no_user": ("🔗 Учётная запись, к которой ведёт ссылка, не активна.\n\n"
                "Обратитесь к тренеру — это он поправит."),
}
